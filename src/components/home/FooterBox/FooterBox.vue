  ...
  <Transition name="fade">
    <div ref="containerRef"
      ...
      <div class="grid grid-cols-2 text-sm gap-x-3 gap-y-8 lg:w-2/3 sm:grid-cols-4">
		...
		<div class="text-center">© 1968 Company Co. All rights reserved.</div>
</template>
<script setup>
...
function handleScroll(event) {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (direction > 0 && !isContentVisible.value) {
      isContentVisible.value = true;
    } else if (direction < 0 && isContentVisible.value) {
...
function handleTransitionEnd(e) {
  if (e.propertyName === 'opacity' && e.target === containerRef.value) {
    scrollToBottom(); // 只有当目标元素是容器并且过渡是关于透明度时，才滚动到底部
  }
}
// 组件挂载时添加事件监听器
onMounted(() => {
